...
export function TopBar() {
  ...
  return (
    <header className="top-bar">
      <div className="container top-bar-inner">
        <Link to="/" className="top-bar-brand" onClick={closeMenu}>
          <svg className="top-bar-logo" width="24" height="24" viewBox="0 0 32 32" fill="none" aria-hidden="true">
            <rect x="4" y="22" width="24" height="5" rx="1" fill="#4a6fe0"/>
            <rect x="6" y="15" width="20" height="5" rx="1" fill="#f2a93b"/>
            <rect x="8" y="8" width="16" height="5" rx="1" fill="#e15a4b"/>
        ...
        <button
          ref={btnRef}
          type="button"
          className={`top-bar-menu-btn ${menuOpen ? 'is-open' : ''}`}
          onClick={() => setMenuOpen((open) => !open)}
          aria-expanded={menuOpen}
          aria-label={menuOpen ? 'Close menu' : 'Open menu'}
        >
        ...
        {session ? (
          <div className="top-bar-account">
            ...
            {canAccessAdmin && (
              ...
              >
                <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12 2 4 5v6c0 5 3.4 8.7 8 10 4.6-1.3 8-5 8-10V5l-8-3Z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round"/>
      ...
      {menuOpen && (
        <div ref={menuRef} className="top-bar-mobile-dropdown">
          <div className="container">
            <nav className="top-bar-mobile-menu">
              ...
              {session && canAccessAdmin && (
                <NavLink to="/admin" onClick={closeMenu} className={({ isActive }) => `top-bar-mobile-link ${isActive ? 'is-active' : ''}`}>
                  Admin
                </NavLink>
              )}
              {session && (
                <NavLink to="/wishlist" onClick={closeMenu} className={({ isActive }) => `top-bar-mobile-link ${isActive ? 'is-active' : ''}`}>
                  Wishlist
                  {wishlistCount > 0 && <span className="top-bar-mobile-badge">{wishlistCount}</span>}
                </NavLink>
              )}
              {session && (
                <NavLink to="/cart" onClick={closeMenu} className={({ isActive }) => `top-bar-mobile-link ${isActive ? 'is-active' : ''}`}>
                  Cart
                  {cartCount > 0 && <span className="top-bar-mobile-badge">{cartCount}</span>}
                </NavLink>
              )}
              {session && (
                <NavLink to="/settings" onClick={closeMenu} className={({ isActive }) => `top-bar-mobile-link ${isActive ? 'is-active' : ''}`}>
                  Settings
                </NavLink>
              )}
              {session ? (
                ...
                <Link to="/login" onClick={closeMenu} className="top-bar-mobile-link">
                  Log in
                </Link>
              )}
            </nav>
